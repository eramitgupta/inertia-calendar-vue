...
export const eventKey = (event: CalendarEvent): string | number => event.id ?? `${event.date}-${event.start}-${event.title}`

export const eventTimeStyle = (event: CalendarEvent, minimumHeight = 20): Record<string, string> => ({
...
export const sortEventsByTime = (events: CalendarEvent[]): CalendarEvent[] => [...events].sort((a, b) => timeToMinutes(a.start) - timeToMinutes(b.start))
