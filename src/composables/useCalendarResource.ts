...
export function useCalendarResource(props: CalendarProps) {
  ...
  const mentionUsers = computed(() => props.mentionUsers ?? config.value.mentionUsers ?? resource.value?.mentionUsers ?? [])
  ...
  const shouldPersist = computed(() => Boolean(props.persistWithInertia ?? config.value.persistWithInertia ?? hasRoutes.value))
  ...
  const calendarOptions = computed<CalendarOptions>(() => ({
    ...
    sidebarOpen: props.sidebarOpen ?? config.value.sidebarOpen ?? config.value.sidebar ?? resource.value?.data?.sidebarOpen,
  }))
